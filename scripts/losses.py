...
def get_inverse_propagation_loss_fn(config, train, device, forward_propagator):
    """Create the inverse-propagation reconstruction loss for optical forward propagation."""

    sigma = config.model.sigma
    # Sample the forward diffraction step for each training example.
    ...
    def loss_fn(model, batch):
        ...
        # Draw forward-step labels for the current batch.
        fwd_steps = label_sampling_fn(batch.shape[0], batch.device)
        
        # Generate observations at the current and previous time steps via optical forward propagation.
        blurred_batch = forward_propagator(batch, fwd_steps).float()
        less_blurred_batch = forward_propagator(batch, fwd_steps-1).float()
...
def get_step_fn(train, config, optimize_fn=None,
                forward_propagator=None, device=None):
    """A wrapper for loss functions in training or evaluation
    Based on code from https://github.com/yang-song/score_sde_pytorch"""
    if device == None:
        device = config.device

    loss_fn = get_inverse_propagation_loss_fn(config, train,
                                       device, forward_propagator=forward_propagator)
